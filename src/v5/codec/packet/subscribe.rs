...
mod tests {
    ...
    fn test_sub_ack() {
        let ack = SubscribeAck {
            packet_id: NonZeroU16::new(1).expect(""),
            properties: Vec::new(),
        ...
        ack.encode(&mut buf, size as u32).expect("");
        assert_eq!(ack, SubscribeAck::decode(&mut buf.freeze()).expect(""));
        ...
        let size = ack.encoded_size(99999);
        ack.encode(&mut buf, size as u32).expect("");
        assert_eq!(ack, UnsubscribeAck::decode(&mut buf.freeze()).expect(""));
